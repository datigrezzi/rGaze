...
remove.blinks <- function(x, ts, norm_threshold = 0.04, initial_interpolation_maxgap=2, sampling_rate = 50, blink_duration_range = c(100, 400)){
	...
	idx[abs(vel) <= 0.01, 2] <- 3 # almost zero velocity
	...
	for(i in 1:length(idx_neg_spikes)){
		...
		nextZeroIdxs <- idx[idx[,3] >= (thisBlinkOnset + blink_duration_range[1]) & idx[,3] <= (thisBlinkOnset + blink_duration_range[2]) & idx[,2] == 3, 1]
	...
	# if current is NA for a duration of blink, until next near-zero
